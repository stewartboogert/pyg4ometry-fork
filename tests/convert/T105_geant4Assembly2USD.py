...
def Test(vis=False, interactive=False, usd=True, outputPath=None, refFilePath=None):
    if not outputPath:
    ...
    w = _gd.Writer()
    w.addDetector(reg)
    w.write(outputPath / "T105_geant4Assembly2USD.gdml")

    # USD conversion
    if usd:
        stage = _convert.geant42Geant4USD.geant4Reg2Geant4USDStage(reg)
        stage.GetRootLayer().Export(str(outputPath / "T105_geant4Assembly2USD.usda"))
